...
void GetCurveForImage(const Mat& filename, vector<Point>& curve, bool onlyUpper = true, bool getLower = false);
...
template<typename T>
void ShowMathMLCurve(const vector<Point_<T> > a_canon, const std::string& title)
{
#ifdef HAVE_MATHGL
	mglGraph gr;
...
template<typename T>
void ShowMathMLCurve(const vector<T> a_canon, const std::string& title)
{
...
template<typename T>
vector<pair<char,int> > CurvatureExtrema(const vector<Point_<T> >& curve, vector<Point_<T> >& smooth, double cutoff = 0.025, double smooth_sigma = 4.0) {
	vector<double> kappa1;
	ComputeCurveCSS(curve, kappa1, smooth, smooth_sigma, true);
	
	ShowMathMLCurve(kappa1, "curvature");
					  ...
					  const vector<pair<char,int> >& stringrep) 
{
	...
	putText(outout, ss.str(), Point(10,src.rows-20), CV_FONT_NORMAL, 0.5, Scalar(0,0,255), 1);
	stringstream winname; winname << "output" << win_id++;
